...
fn all_guide_commands() -> Vec<GuideCommandEntry> {
    vec![
        ...
        GuideCommandEntry {
            path: "mail list",
            topic: "mail",
            summary: "Показать список писем в выбранной папке.",
            requires_account: true,
            examples: vec!["yacli mail list --folder INBOX --limit 10"],
        },
        GuideCommandEntry {
            path: "mail search",
            topic: "mail",
            summary: "Найти письма по текстовому запросу в выбранной папке.",
            requires_account: true,
            examples: vec!["yacli mail search --folder INBOX --query \"Budget\" --limit 10"],
        },
        GuideCommandEntry {
            path: "mail reply",
            topic: "mail",
            summary: "Ответить на письмо по UID с сохранением thread headers.",
            requires_account: true,
            examples: vec!["yacli mail reply --folder INBOX --uid 1353 --text \"Принято\""],
        },
        GuideCommandEntry {
            path: "mail forward",
            topic: "mail",
            summary: "Переслать письмо по UID как inline-forward с явным списком пропущенных вложений.",
            requires_account: true,
            examples: vec![
        ...
        GuideCommandEntry {
            path: "mail read",
            topic: "mail",
            summary: "Открыть одно письмо по UID с декодированным телом и вложениями.",
            requires_account: true,
            examples: vec!["yacli mail read --folder INBOX --uid 1353"],
        },
        GuideCommandEntry {
            path: "mail send",
            topic: "mail",
